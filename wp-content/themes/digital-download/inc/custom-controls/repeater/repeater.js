/*jshint -W065 */
var digitaldownloadRepeaterRow = function( rowIndex, container, label ){
	'use strict';
...
wp.customize.controlConstructor['digital-download-repeater'] = wp.customize.Control.extend({
	ready: function(){
		'use strict';
	...
	addRow: function( data ){
		...
		if ( template ) {
			...
			newRow = new digitaldownloadRepeaterRow(
				control.currentIndex,
    ...
    $(document).on( 'click', 'input[type="font"]', function(){
        ...
        if( ( ! $this.hasClass('ajax-running') ) && $this.siblings( '.font-awesome-list' ).length < 1 ){
            $.ajax({
				...
				url : ajaxurl,
				data: {
					action             : 'digital_download_get_fontawesome_ajax',
					digital_download_customize_nonce: digital_download_customize.nonce
				},
				beforeSend: function(){
